...
Implementation details and contributor notes for the orchestration skill. End-user setup and command reference live in [`README.md`](./README.md); AI / agent-facing instructions live in [`SKILL.md`](./SKILL.md).

## GitHub auth fallback

`bot-review-wait` and `ci-wait` share `scripts/lib/gh-auth.sh::orchestration_sanitize_gh_env` to handle the case where a stale `GH_TOKEN` / `GITHUB_TOKEN` masks working `gh` keyring auth. The ladder is:
