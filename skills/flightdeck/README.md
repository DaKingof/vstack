...
| `pane-poll` | Bell + per-harness adapter (opencode `/session/<id>/message`, claude JSONL tail, `pi-bridge history`, `codex-bridge turns`) or tmux capture-pane fallback + classify. Accepts either legacy `<session>:<window> <pane-index>` or immutable `%N` pane ids directly so polling survives harness window renames. Registry and spawn-file adapter fallbacks both run the same freshness gates before using bridge metadata |
