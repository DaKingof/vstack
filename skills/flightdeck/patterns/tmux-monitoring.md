...
| opencode | `GET /session/<id>/message` → last assistant text. Daemon also polls `GET /question` for the question tool, backing off unchanged polls up to `FD_OC_BACKOFF_MAX_SEC` and resetting on hash change or bell marker. | `tmux capture-pane -p -S -200` |
