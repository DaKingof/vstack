...
> **MODE SWITCH**: Loading this skill puts you in **master mode**. You are NOT a dev agent. You do not write code in any worktree, you do not run cargo/npm/builds, and you do NOT invoke per-issue orchestration workflows directly. Your role is **observe-and-direct**: from the moment a user invokes flightdeck's `start` from main, you own the master arc — dashboard → research/plan evaluation → spawn (`open-terminal`) → watch loop → merge planning → unwind. Per-issue work happens entirely inside the spawned panes; you observe their prompts via the per-harness adapter (opencode panes go through the HTTP-attach adapter — `pane-poll` fetches `/session/<id>/message`, `pane-respond` calls `opencode run --attach`; tmux `capture-pane` / `send-keys` is the fallback path only, used for harnesses without an adapter or for panes whose bridge metadata is unavailable). You query PR/issue facts via `gh` and `linear` and direct the per-issue agents by sending responses to their prompts. You never run `bot-review-wait`, `ci-wait`, `merge-pr`, or any other orchestration workflow yourself.
>
...
| `flightdeck-daemon` | External bash wake driver. Per-pane subscribers (opencode HTTP-attach in Phase 1) emit normalized turn-end events into a wake-events log; daemon main loop drains and routes canonical-tag events through wake_master. Panes without an adapter fall through to the legacy capture-pane / bell / hash-stable loop. Coalesces multiple ready panes into one wake. Writes events JSONL master can drain. Actions: `start \| stop \| status \| events \| ack` |
...
| `pane-poll` | Single-window status read: bell flag + per-harness adapter (opencode → `GET /session/<id>/message`) or tmux `capture-pane` fallback + classify |
| `pane-respond` | Send a response to a pane. Three modes: positional `<payload>` for free-text, `--option N` for numeric option pick (harness-aware: Claude Code uses arrow navigation; opencode adapter sends digits as text), `--keys k1,k2,...` for multi-step forms (rejected for opencode adapter unless `--keys-allow-tmux`). Opencode panes route through `opencode run --attach --format json` when registry has bridge metadata; falls back to tmux paste-buffer when unavailable. Validates rebase-multi-choice payloads include the preserve/apply/verify triplet |
