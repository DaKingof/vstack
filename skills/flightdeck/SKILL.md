...
| `flightdeck-dashboard` | Rust/ratatui dashboard. `tui --demo[=NAME]` uses compiled fixtures; `tui --state-file <path>` reads real master-state JSON; `tui --session <name>` resolves the project state file and falls back to newest valid terminated archive. Other subcommands are reserved and exit `2` until later phases. |
